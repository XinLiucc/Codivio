...
databases:
  ...
  2: # 系统缓存
    ...
    keys:
      ...
      - "project:filetree:{projectId}"     # 项目文件树缓存
      - "file:info:{fileId}"               # 文件信息缓存
      - "file:content:{fileId}"            # 文件内容缓存
...
-- 5. 根据项目路径获取文件ID（项目服务查询文件服务）
SELECT 
    ft.file_id
FROM project_file_tree ft
WHERE ft.project_id = ?
  AND ft.file_path = ?
  AND ft.type = 'file';
```
...
CREATE INDEX idx_project_file_tree_project_path ON project_file_tree(project_id, file_path);
