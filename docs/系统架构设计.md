...
技术栈:
  - Spring Boot 3.x + 文件上传
  - 本地文件系统/对象存储
  - MySQL (文件元数据和版本)
  - Redis (文件内容缓存)

Docker镜像: codivio/file-service:latest
```

#### 5. 网关服务 (Gateway Service) - 端口 8080
```yaml
业务职责:
  - 统一入口路由
  - 负载均衡
...
public class UserController {
    ...
    @GetMapping("/api/v1/users/{userId}/projects")
